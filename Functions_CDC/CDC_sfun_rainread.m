...
% 	  table=[table; table(end,1)+tablestep -1]; %tablestep length of time step in table
      ...
      if strcmp(crep,'on')
          tNew=mod(t, table(len, 1))-tstep; %eventStart start TIME in table
      else
          tNew=t;
      end
      
      rainTimeStep = table(2,1) - table(1,1);

      % Output value, in mm/h
      retvalNeno = interp1(table(:,1), table(:,2), tNew)/rainTimeStep*3600;
      sys=retvalNeno;
