...
A=[simtime,vol];
step = simtime(2)-simtime(1);
if(round(step)~=300)
    error('Time step in ixx files has to be 5 minutes')
end
% step=300; %use timstep calc from mse instead?
% if(simtime(2)-simtime(1)~=step)
%     warning('ixx read only works with 5 minutes timestep')
% end
